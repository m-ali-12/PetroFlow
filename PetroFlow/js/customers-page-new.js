...
function updateSummary() {
  ...
  // ── Daily Pending Udhaar Card (from machine reading) ──────────
  updateDailyUdhaarCard();
}

async function updateDailyUdhaarCard() {
  const card = $('daily-udhaar-card');
  if (!card) return; // card HTML nahi hai toh skip

  // UdhaarReminder module available hai?
  if (!window.UdhaarReminder?.getDailySummary) {
    card.style.display = 'none';
    return;
  }

  try {
    const s = await window.UdhaarReminder.getDailySummary();
    if (!s || !s.hasReadings || s.machineUdhaar <= 0) {
      card.style.display = 'none';
      return;
    }

    card.style.display = '';
    const pct = Math.min(100, Math.round((s.enteredAmount / s.machineUdhaar) * 100));
    const allDone = s.pendingAmount <= 1;
    const barColor = allDone ? '#198754' : pct >= 50 ? '#fd7e14' : '#dc3545';

    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:6px;">
        <div>
          <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#888;margin-bottom:4px;">
            ${allDone ? '✅' : '⚠️'} Aaj Ka Udhaar
          </div>
          <div style="font-family:'Courier New',monospace;font-size:1.1rem;font-weight:800;color:${allDone ? '#198754' : '#dc3545'};">
            Rs. ${fmt(s.pendingAmount)}
          </div>
          <div style="font-size:10px;color:#aaa;margin-top:2px;">
            ${allDone
              ? '✅ Sab customers enter ho gaye!'
              : `Pending — Machine: Rs.${fmt(s.machineUdhaar)} | Entered: Rs.${fmt(s.enteredAmount)}`
            }
          </div>
        </div>
        <div style="text-align:right;">
          <div style="font-size:11px;font-weight:700;color:${barColor};">${pct}%</div>
          <div style="font-size:10px;color:#aaa;">${s.uniqueCustomers} customer${s.uniqueCustomers !== 1 ? 's' : ''}</div>
        </div>
      </div>
      <!-- Progress -->
      <div style="margin-top:8px;background:#f0f0f0;border-radius:20px;height:6px;overflow:hidden;">
        <div style="height:100%;border-radius:20px;background:${barColor};width:${pct}%;transition:width .6s;"></div>
      </div>
      ${!allDone ? `
        <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;">
          <a href="transactions.html" style="background:#0d6efd;color:#fff;border:none;border-radius:5px;padding:4px 10px;font-size:11px;font-weight:700;text-decoration:none;">
            ➕ Credit Sale Enter Karein
          </a>
          <button onclick="window.UdhaarReminder.dismissSession('${s.date}');this.closest('[id=daily-udhaar-card]').style.display='none';"
            style="background:#f0f0f0;color:#555;border:none;border-radius:5px;padding:4px 10px;font-size:11px;cursor:pointer;">
            ✅ Sab Ho Gaye
          </button>
        </div>
      ` : ''}
    ...
    console.warn('Daily udhaar card error:', e);
