...
// // new code for cleaning the transaction history
// // ============================================================
// // transactions-COMPLETE.js  —  FINAL VERSION (v3)
// // Features:
// //   ✅ Duplicate fix (ID-based dedup)
...
// })();
// previous 824 i thnk it near 

// agian chagne
// ============================================================
// transactions-COMPLETE.js — FINAL v4
// FIXES: checkbox visible, type badge color forced inline
// ============================================================
(function () {
  'use strict';
  if (document.body.getAttribute('data-page') !== 'transactions') return;
  ...
  function showToast(type, title, msg) {
    const t = el('liveToast'); if(!t) return;
    el('toast-title').textContent = title;
  ...
  async function loadCustomers() {
    try {
      ...
      ['sale-customer'].forEach(id=>{
        ...
        s.innerHTML='<option value="">Select Customer</option>'+allCustomers.map(c=>`<option value="${c.id}">${c.sr_no} - ${c.name}</option>`).join('');
      });
      const vs=el('vasooli-customer');
      if(vs)vs.innerHTML='<option value="">Select Customer</option>'+allCustomers.filter(c=>c.category!=='Owner').map(c=>`<option value="${c.id}">${c.sr_no} - ${c.name}</option>`).join('');
  ...
  function openPrint(txns,mode){
    ...
    const TFOOT=`<tr style="background:#eaf0fb;font-weight:700;border-top:2px solid #1a5276">
      ...
      <td style="padding:5px 6px;text-align:right">Rs.${fmt(totCr+totDb+totEx)}</td>
      <td style="padding:5px 6px;text-align:right;color:#198754">Rs.${fmt(totCr)}</td>
      <td style="padding:5px 6px;text-align:right;color:#0d6efd">Rs.${fmt(totDb+totEx)}</td>
      <td></td>
    </tr>`;
...
<div class="footer"><span>${company} — Official Receipt</span><span>Generated: ${new Date().toLocaleString('en-PK')}</span></div>
  ...
  // ── Init ───────────────────────────────────────────────────
  document.addEventListener('DOMContentLoaded',async()=>{
    console.log('Transactions v4 init...');
    setupEvents();
    await loadFuelPrices();
    await loadCustomers();
    await loadTransactions();
    console.log('Transactions v4 ready.');
  });
