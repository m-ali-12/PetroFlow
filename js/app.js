...
function showToast(msg,type='info'){
 ...
 $('toast-title').textContent=type.toUpperCase();
...
function updateStockDisplay(){
 ...
 const petrol=tanksCache.find(t=>t.fuel_type==="Petrol");
 const diesel=tanksCache.find(t=>t.fuel_type==="Diesel");

 if(petrol && $('petrol-stock'))
  $('petrol-stock').textContent=formatNumber(petrol.current_stock);

 if(diesel && $('diesel-stock'))
  $('diesel-stock').textContent=formatNumber(diesel.current_stock);
}
...
function populateCustomerDropdowns(){
 ...
 if(sale) sale.innerHTML=html;
 if(vasooli) vasooli.innerHTML=html;
}
...
function initPriceCalculation(){
 ...
 const price=$('sale-unit-price');
 const amount=$('sale-amount');

 if(!liters || !price || !amount)return;
...
window.addSale=async function(){
 ...
 const customer=$('sale-customer').value;
 ...
 if(!customer||!fuel||!liters||!price){
  showToast("Fill all fields","error");
  return;
 }

 const tank=tanksCache.find(t=>t.fuel_type===fuel);
