// TRANSACTIONS - COMPLETE PERFECT VERSION
(function() {
'use strict';
...
async function handleNewSale() {
  ...
  isSubmitting = true;
  console.log('Saving:', { customerId, fuelType, liters, unitPrice, amount });

  try {
    // Save with fuel_type column
    const { error } = await supabase
      ...
      .insert([{
        ...
        fuel_type: fuelType, // SAVE FUEL TYPE
...
// ============================================
// VASOOLI - WITH FUEL CATEGORY & DATE
// ============================================
...
// ============================================
// AUTO-CALCULATE WITH SETTINGS PRICES
// ============================================

async function loadFuelPrices() {
  try {
    // Get prices from settings table
    const { data, error } = await supabase
      .from('settings')
    ...
    if (data) {
      localStorage.setItem('fuel_prices', JSON.stringify({
        'Petrol': data.petrol_price || 285,
        'Diesel': data.diesel_price || 305
      }));
...
function setupAutoCalculate() {
  ...
  if (fuelSelect) {
    fuelSelect.addEventListener('change', () => {
      const prices = JSON.parse(localStorage.getItem('fuel_prices') || '{"Petrol":285,"Diesel":305}');
      const price = prices[fuelSelect.value] || 285;
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.body.getAttribute('data-page') === 'transactions') {
    ...
    loadFuelPrices(); // Load prices from settings
    loadInitialTransactions();
    loadCustomers();
    setupAutoCalculate();
    setupFormHandlers();
    
    // Set default date for vasooli to today
    if ($('vasooli-date')) {
