...
(function () {
  ...
  DR.save = async function() {
    ...
    try {
      ...
      // ── Udhaar Session Register ────────────────────────────────
      // Aaj ke saare inserts mein se total udhaar nikalo
      // aur UdhaarReminder module ko batao taake banner show ho sake
      const totalUdhaarToday = inserts.reduce((sum, ins) => {
        try {
          const meta = JSON.parse(ins.description || '{}');
          return sum + (parseFloat(meta.udhaar) || 0);
        } catch(e) { return sum; }
      }, 0);

      if (totalUdhaarToday > 0 && window.UdhaarReminder?.registerSession) {
        window.UdhaarReminder.registerSession(
          date,
          totalUdhaarToday,
          inserts.length === 1 ? inserts[0].fuel_type : 'Mixed'
        );
        showToast('warning', '⚠️ Udhaar Reminder',
          `Rs. ${totalUdhaarToday.toLocaleString('en-PK')} ka udhaar pending — Transaction page pe customers zaroor enter karein!`
        );
        // Refresh banner after short delay (supabase write settle hone do)
        setTimeout(() => {
          if (window.UdhaarReminder?.refreshBanner) window.UdhaarReminder.refreshBanner();
        }, 1200);
      }
      // ───────────────────────────────────────────────────────────
