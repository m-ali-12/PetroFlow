...
(function () {
  ...
  // ── SETTINGS HELPERS ───────────────────────────────────────
  async function getSettings() {
    var r = await supabase.from('settings').select('*').eq('user_id', uid())
  ...
  async function patchSettings(sid, patch) {
    patch.updated_at = new Date().toISOString();
    var r = await supabase.from('settings').update(patch).eq('id', sid).eq('user_id', uid());
    if (r.error) throw r.error;
  }
  ...
  async function loadMobilStock() {
    try {
      ...
      var arrRes  = await supabase.from('mobil_arrivals').select('quantity, category').eq('user_id', uid());
      var saleRes = await supabase.from('mobil_sales').select('quantity, category').eq('user_id', uid());
  ...
  async function loadCustomerDropdown() {
    try {
      var r = await supabase.from('customers').select('id,sr_no,name,category').eq('user_id', uid()).order('sr_no',{ascending:true});
      if (r.error) throw r.error;
  ...
  async function buildAllRecords() {
    ...
    var arrRes  = await supabase.from('mobil_arrivals').eq('user_id', uid())
  ...
  window.addMobilExpense = async function() {
    ...
    try {
      ...
      var tx = await supabase.from('transactions').insert([{ user_id: uid(),
  ...
  window.deleteMobilArrival = async function(id) {
    if (!confirm('Yeh arrival record delete karein?')) return;
    try {
      var r = await supabase.from('mobil_arrivals').delete().eq('id', id).eq('user_id', uid());
      if (r.error) throw r.error;
      showToast('Arrival delete ho gaya!','success');
  ...
  window.deleteMobilSale = async function(id) {
    if (!confirm('Yeh sale record delete karein?')) return;
    try {
      var r = await supabase.from('mobil_sales').delete().eq('id', id).eq('user_id', uid());
      if (r.error) throw r.error;
      showToast('Sale delete ho gaya!','success');
      loadMobilStock(); loadMobilTransactions();
    } catch(e) { showToast('Error: '+e.message,'error'); }
