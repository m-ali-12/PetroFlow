...
(function () {
  ...
  function renderBanner(sessions) {
    ...
    const container = document.querySelector('.container, main, #main-content, body');
    if (!container) return;
    ...
    // Insert at top of container content
    const firstChild = container.querySelector('.container > *:first-child, main > *:first-child')
  ...
  /* ─── Auto-register from daily-readings.js ────────────────
     Patch DR.save to auto-register session after successful save
     This works if udhaar-reminder.js is loaded AFTER daily-readings.js
  ─────────────────────────────────────────────────────────── */
  function patchDailySave() {
    if (!window.DR) { setTimeout(patchDailySave, 200); return; }
    const origSave = window.DR.save;
    if (!origSave || origSave._patched) return;

    window.DR.save = async function() {
      await origSave.apply(this, arguments);

      // After save, recalculate today's machine udhaar and register
      try {
        const summary = await window.UdhaarReminder.getDailySummary(today());
        if (summary && summary.machineUdhaar > 0) {
          window.UdhaarReminder.registerSession(today(), summary.machineUdhaar, 'Mixed');
          // Refresh banner
          setTimeout(loadAndRenderBanner, 800);
        }
      } catch(e) { console.warn('UdhaarReminder patch error:', e); }
    };
    window.DR.save._patched = true;
  }
  ...
  function init() {
    ...
    // Patch DR.save on daily-readings page
