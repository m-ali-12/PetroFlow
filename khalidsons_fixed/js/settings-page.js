...
(function () {
  ...
  async function loadSystemInfo() {
    try {
      const { count: custCount } = await supabase.from('customers').select('id', { count: 'exact', head: true }).eq('user_id', uid());
      const { count: txCount } = await supabase.from('transactions').select('id', { count: 'exact', head: true }).eq('user_id', uid());
  ...
  window.saveMobilPricesWithHistory = async function () {
    ...
    try {
      ...
      if (rowId) {
        ...
        const result = await supabase.from('settings').insert([{ user_id: uid(), mobil_history: history, updated_at: new Date().toISOString() }]);
        error = result.error;
      }
  ...
  window.updateTankCapacity = async function () {
    ...
    try {
      ...
      if (existing?.id) {
        ...
        const result = await supabase.from('settings').insert([{ user_id: uid(), petrol_capacity: petrolCap, diesel_capacity: dieselCap }]);
        error = result.error;
      }
      if (error) { alert('Error: ' + error.message); return; }
  ...
  window.clearOldData = async function () {
    ...
    try {
      const { error } = await supabase.from('transactions').delete().eq('user_id', uid()).lt('created_at', oneYearAgo.toISOString());
      if (error) { alert('Error: ' + error.message); return; }
